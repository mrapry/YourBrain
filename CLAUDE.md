...
proactively - do not wait to be asked. Auto-capture hooks (`yb hook`) also record
session activity in the background.

## Context-first policy - always check YourBrain first

YourBrain is the FIRST place to look for context, not the last.
...
- ONLY if `yb_recall` returns nothing relevant do you proceed with your own
  actions. Once you discover the answer, persist it with `yb_remember` so the
...
Persisting knowledge is SAFE - the engine deduplicates and conflict-checks every
write. Call `yb_remember` whenever the user states or confirms anything that
could matter later: decisions, facts, conventions, preferences, constraints,
environment/config details, or a solved bug.
...
Most writes just succeed. If `yb_remember` returns `"status":"conflict"`, the
engine needs a human decision - do NOT force it. Present the existing vs. new
content and the detected relation, recommend a sensible default (`supersede` ->
replace, `duplicate` -> discard_new, `contradicts` -> ask the user), then call
`yb_resolve` with the `conflict_id` and the chosen action (`replace` |
